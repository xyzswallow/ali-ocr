...
export async function POST(request: Request) {
  ...
  let recordId: string;
  try { recordId = createInvoiceRecord(file); }
  ...
  try {
    ...
    completeInvoiceRecord(recordId, result.requestId, result.data, result.rawResponse);
    return NextResponse.json({ recordId, ...result });
  } catch (error) {
    const message = error instanceof OcrServiceError ? error.message : "发票识别失败，请稍后重试。";
    try { failInvoiceRecord(recordId, message); } catch { /* Preserve the OCR error. */ }
